        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-muted-foreground font-body mt-6 max-w-2xl mx-auto text-lg">
              Kein Verkaufsgespräch. Kein Druck. Nur eine ehrliche Analyse deiner aktuellen Situation und ein klarer Plan.
            </p>
          </div>

          <div className="bg-card border border-primary/30 p-10 md:p-12 rounded-lg text-center mb-10">
            ...
            <p className="text-muted-foreground font-body mb-8 leading-relaxed max-w-xl mx-auto">
              In einem kurzen Gespräch schauen wir gemeinsam auf deine aktuelle Situation.
            </p>
            <a
            ...
            <p className="text-muted-foreground text-sm font-body mt-4">Kurz sprechen, Setup verstehen, nächsten Schritt klären</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 pt-12 border-t border-border">
            {[
              ...
              { icon: Phone, label: "WhatsApp", value: "0152 23856537", href: "https://wa.me/4915223856537", external: true },
              { icon: MapPin, label: "Standort", value: "Bielefeld, Deutschland", href: null },
            ].map((item, index) => (
              <div key={index} className="text-center group">
                <div className="inline-flex items-center justify-center w-14 h-14 border border-border group-hover:border-primary/50 transition-colors mb-4 rounded-lg">
                  <item.icon className="w-6 h-6 text-primary" />
                </div>
                <div className="text-muted-foreground text-sm tracking-wide uppercase font-body mb-1">{item.label}</div>
                {item.href ? (
                  <a href={item.href} className="text-foreground font-body hover:text-primary transition-colors" {...(item.external ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                    {item.value}
                  </a>
                ) : (
